...
int main() {
    ...
    const wchar_t* targetProcess = L"java.exe";

    DWORD processId = GetProcessId(targetProcess);
    if (!processId) {
        MessageBox(NULL, L"Не удалось найти процесс java.exe", L"Ошибка", MB_ICONERROR);
        return 1;
    }

    HANDLE hProcess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, processId);
    if (!hProcess) {
        MessageBox(NULL, L"Не удалось открыть процесс java.exe", L"Ошибка", MB_ICONERROR);
        return 1;
    }
